...
class AccountInvoiceReportCustom(models.Model):
    ...
    _description = "Invoices Statistics Custom"
    _auto = False
    _rec_name = 'date'
    ...
    no_invoice = fields.Char(string='Invoice', readonly=True)
    ...
    orderdate = fields.Date(string='OrderDate',readonly=True)
    payref = fields.Char(string='PayRef', readonly=True)
    payby = fields.Char(string='Payby', readonly=True)
    paydate = fields.Date(string='PaymentDate', readonly=True)
    standard_price = fields.Float(string='HPP', readonly=True)
    margin = fields.Float(string='Margin', readonly=True)
    payamount = fields.Float(string='PayAmount', group_operator = 'avg', readonly=True)
    price_subtotal = fields.Float(string='Sub Total', readonly=True)
    so_price = fields.Float(string='SO Price', readonly=True)
    ...
    def _select(self):
        select_str = """
            SELECT sub.id, sub.date, sub.product_id, sub.partner_id, sub.country_id, sub.account_analytic_id,
                ...
                sub.no_invoice, sub.label, sub.discount, sub.price_unit, sub.tax_base_amount, sub.origin, sub.orderdate, sub.payref, sub.paydate, sub.payby, sub.standard_price, sub.margin, sub.payamount, sub.price_subtotal
                , sub.so_price
        """
        return select_str
    ...
    def _sub_select(self):
        select_str = """
                SELECT ail.id AS id,
                    ...
                    sum(ail.price_total-ail.price_subtotal) as tax_base_amount,
                    ai.origin as origin,
                    so.date_order as orderdate,
                    apay.move_name as payref,
                    apay.payment_date as paydate,
                    aj.name as payby,
                    ip.value_float as standard_price,
                    sum(ail.price_unit-ip.value_float) as margin,
                    apay.amount as payamount,
                    ail.price_subtotal,
    ...
    def _from(self):
        from_str = """
                ...
                LEFT JOIN res_partner partner ON ai.commercial_partner_id = partner.id
                LEFT JOIN res_partner partner_ai ON ai.partner_id = partner_ai.id
                LEFT JOIN product_product pr ON pr.id = ail.product_id
                LEFT JOIN product_template pt ON pt.id = pr.product_tmpl_id
                LEFT JOIN product_uom u ON u.id = ail.uom_id
                LEFT JOIN product_uom u2 ON u2.id = pt.uom_id
                
                LEFT JOIN (select distinct on (invoice_id) * from account_invoice_payment_rel order by invoice_id) as aipr ON aipr.invoice_id = ai.id
				LEFT JOIN account_payment apay ON apay.id = aipr.payment_id
                LEFT JOIN account_journal aj ON aj.id = apay.journal_id
                ...
                LEFT JOIN ir_property ip ON (ip.name='standard_price' AND ip.res_id= 'product.product,'||pr.id)
                JOIN (
