...
class aspl_stock_inventory_report(models.AbstractModel):
    _name = 'report.aspl_stock_inventory_report.report_stock_inventory'
    _description = 'Report Stock Inventory'

    @api.model
    def get_report_values(self, docids, data=None):
        report = self.env['ir.actions.report']._get_report_from_name('aspl_stock_inventory_report.report_stock_inventory')
        ...
        self._cr.execute(''' 
                        ...
                        FROM
                            ...
                            SELECT pp.id, pp.default_code,m.date,
                                ...
                                CASE when pt.uom_id = m.product_uom
                                THEN coalesce(round(sum(-m.product_qty),5)::decimal, 0.0)
                                ELSE coalesce(round(sum(-m.product_qty * pu.factor / u.factor ),5)::decimal, 0.0) 
                                END AS qty
        ...
        if product_data:
            ...
            self._cr.execute('''
                            SELECT pp.id AS product_id,pt.categ_id,
                                ...
                                THEN -round((sm.product_qty * pu.factor / pu2.factor),5)
                                ELSE 0.0 
                                END
                                )) AS product_qty_out,
                                ...
                                sum((
                                CASE WHEN (spt.code ='internal' OR spt.code IS NULL) AND spt.is_stock = False AND sm.location_dest_id IN %s AND sourcel.usage ='internal' AND destl.usage ='internal' AND sourcel.is_wip = False AND destl.is_wip = False
                                THEN round((sm.product_qty * pu.factor / pu2.factor),5)  
                                WHEN (spt.code ='internal' OR spt.code IS NULL) AND spt.is_stock = False AND sm.location_id IN %s AND sourcel.usage ='internal' AND destl.usage ='internal' AND sourcel.is_wip = False AND destl.is_wip = False
                                THEN -round((sm.product_qty * pu.factor / pu2.factor),5)
                                ELSE 0.0 
                                END
                                ...
                                sum((
                                CASE WHEN spt.code in ('outgoing', 'internal') AND sp.is_pinjam = True AND sm.location_id in %s AND sourcel.usage !='inventory' AND destl.usage !='inventory'
                                THEN -round((sm.product_qty * pu.factor / pu2.factor),5)
                                ELSE 0.0 
                                END
                                )) AS product_qty_pinjamout,
                                
                                sum((
                                CASE WHEN spt.code in ('incoming', 'internal') AND sp.is_pinjam = True AND sm.location_dest_id in %s AND sourcel.usage !='inventory' AND destl.usage !='inventory' 
                                THEN round((sm.product_qty * pu.factor / pu2.factor),5)
                                ELSE 0.0 
                                END
                                )) AS product_qty_pinjamin,

                                sum((
                                CASE WHEN sourcel.usage = 'internal' AND sm.location_dest_id in %s AND sourcel.is_wip = True
                                THEN round((sm.product_qty * pu.factor / pu2.factor),5)
                                ELSE 0.0 
                                END
                                )) AS product_qty_returpro,

                                sum((
                                CASE WHEN destl.usage ='internal' AND sm.location_id in %s AND destl.is_wip = True
                                THEN -round((sm.product_qty * pu.factor / pu2.factor),5)
                                ELSE 0.0 
                                END
                                )) AS product_qty_production,
                            ...
                            ''', (tuple(locations), tuple(locations), tuple(locations), tuple(locations), tuple(locations), tuple(locations), tuple(locations), tuple(locations), tuple(locations), tuple(locations), tuple(locations), tuple(locations), start_date, end_date, product_data))
                ...
                if not record.with_zero:
                    ...
                    for key, value in records_by_categories.items():
                        for each in value:
                            ...
                            today_movment_total = each.get('product_qty_in') + each.get('product_qty_inspect') + each.get('product_qty_pinjamin') + each.get('product_qty_pinjamout') + each.get('product_qty_returpro') + each.get('product_qty_production') + each.get('product_qty_adjustment') + each.get('product_qty_out')
