...
class stock_inventory_wizard(models.TransientModel):
    ...
    def generate_pdf_report(self):
        ...
        return self.env.ref('aspl_stock_inventory_report.action_report_stock_inventory').report_action(self, data=datas)

    def generate_xls_report(self):
        self.check_date_range()
        report_stock_inv_obj = self.env['report.aspl_stock_inventory_report.report_stock_inventory']
        ...
        for warehouse in self.warehouse_ids:
            ...
            lower_header_lst = ['SKU', 'Product', 'Ne', 'Cost', 'Beginning', 'Received', 'Sales'
                                # , 'Internal'
                                , 'Inspect', 'Pinjamin', 'Pinjamout', 'Returpro', 'Production', 'Adjustments', 'Ending', 'Value']
            # row = 3
            row = 0
            ...
            if not self.group_by_categ:
                ...
                for product in product_ids:
                    ...
                    today_movment_qty = product_val.get('product_qty_in') + product_val.get('product_qty_out')\
                        + product_val.get('product_qty_pinjamin') + product_val.get('product_qty_pinjamout')\
                        + product_val.get('product_qty_inspect') + product_val.get('product_qty_returpro')\
                        + product_val.get('product_qty_production') + product_val.get('product_qty_adjustment')
                    ...
                    if not self.with_zero and beginning_qty == 0.0 and product_val.get('product_qty_in') == 0.0 and product_val.get('product_qty_out') == 0.0 and \
                        product_val.get('product_qty_inspect') == 0.0 and product_val.get('product_qty_pinjamin') == 0.0 and product_val.get('product_qty_pinjamout') == 0.0 and \
                        product_val.get('product_qty_internal') == 0.0 and product_val.get('product_qty_returpro') == 0.0 and \
                        product_val.get('product_qty_production') == 0.0 and product_val.get('product_qty_adjustment') == 0 :
                        continue
                    ...
                    # worksheet.write(rows, 6, product_val.get('product_qty_internal'), number_format)
                    worksheet.write(rows, 7, product_val.get('product_qty_inspect'), number_format)
                    worksheet.write(rows, 8, product_val.get('product_qty_pinjamin'), number_format)
                    worksheet.write(rows, 9, product_val.get('product_qty_pinjamout'), number_format)
                    worksheet.write(rows, 10, product_val.get('product_qty_returpro'), number_format)
                    worksheet.write(rows, 11, product_val.get('product_qty_production'), number_format)
                    worksheet.write(rows, 12, product_val.get('product_qty_adjustment'), number_format)
                    worksheet.write(rows, 13, ending_qty, number_format)
                    worksheet.write(rows, 14, val_qty, number_format)
                    ...
                    prod_qty_pjmin += product_val.get('product_qty_pinjamin')
                    prod_qty_pjmout += product_val.get('product_qty_pinjamout')
                    prod_qty_rpro += product_val.get('product_qty_returpro')
                    prod_qty_pro += product_val.get('product_qty_production')
                ...
                worksheet.write(rows + 1, 7, prod_qty_ins, style=header_style)
                worksheet.write(rows + 1, 8, prod_qty_pjmin, style=header_style)
                worksheet.write(rows + 1, 9, prod_qty_pjmout, style=header_style)
                worksheet.write(rows + 1, 10, prod_qty_rpro, style=header_style)
                worksheet.write(rows + 1, 11, prod_qty_pro, style=header_style)
                worksheet.write(rows + 1, 12, prod_qty_adjust, style=header_style)
                worksheet.write(rows + 1, 13, prod_ending_qty, style=header_style)
                worksheet.write(rows + 1, 14, val_ending_qty, style=header_style)
            else:
                rows += 1
                product_val = report_stock_inv_obj.get_product_sale_qty(self)
                for categ, product_value in product_val.items():
                    categ_prod_beginning_qty = categ_prod_qty_in = categ_prod_qty_out = categ_prod_qty_int = categ_prod_qty_ins = categ_prod_qty_pjmin = categ_prod_qty_pjmout = categ_prod_qty_rpro = categ_prod_qty_pro = categ_prod_qty_adjust = categ_prod_ending_qty = categ_val_ending_qty = 0.00
                    ...
                    for product in product_value:
                        ...
                        ending_qty = beginning_qty + product.get('product_qty_in') + product.get('product_qty_out') \
                            + product.get('product_qty_inspect') + product.get('product_qty_returpro') \
                            + product.get('product_qty_pinjamin') + product.get('product_qty_pinjamout') \
                            + product.get('product_qty_production') + product.get('product_qty_adjustment')
                        if not self.with_zero and beginning_qty == 0.0 and product.get('product_qty_in') == 0.0 and product.get('product_qty_out') == 0.0 and \
                            product.get('product_qty_internal') == 0.0 and product.get('product_qty_inspect') == 0.0 and product.get('product_qty_returpro') == 0.0 and \
                            product.get('product_qty_pinjamin') == 0.0 and product.get('product_qty_pinjamout') == 0.0 and \
                            product.get('product_qty_production') == 0.0 and product.get('product_qty_adjustment') == 0.0:
                            continue
                        ...
                        # worksheet.write(rows, 6, product.get('product_qty_internal'), number_format)
                        worksheet.write(rows, 7, product.get('product_qty_inspect'), number_format)
                        worksheet.write(rows, 8, product.get('product_qty_pinjamin'), number_format)
                        worksheet.write(rows, 9, product.get('product_qty_pinjamout'), number_format)
                        worksheet.write(rows, 10, product.get('product_qty_returpro'), number_format)
                        worksheet.write(rows, 11, product.get('product_qty_production'), number_format)
                        worksheet.write(rows, 12, product.get('product_qty_adjustment'), number_format)
                        worksheet.write(rows, 13, ending_qty, number_format)
                        worksheet.write(rows, 14, val_qty, number_format)
                        ...
                        categ_prod_qty_pjmin += product.get('product_qty_pinjamin')
                        categ_prod_qty_pjmout += product.get('product_qty_pinjamout')
                        categ_prod_qty_rpro += product.get('product_qty_returpro')
                        categ_prod_qty_pro += product.get('product_qty_production')
                    ...
                    worksheet.write(rows, 7, categ_prod_qty_ins, style=total_value_style)
                    worksheet.write(rows, 8, categ_prod_qty_pjmin, style=total_value_style)
                    worksheet.write(rows, 9, categ_prod_qty_pjmout, style=total_value_style)
                    worksheet.write(rows, 10, categ_prod_qty_rpro, style=total_value_style)
                    worksheet.write(rows, 11, categ_prod_qty_pro, style=total_value_style)
                    worksheet.write(rows, 12, categ_prod_qty_adjust, style=total_value_style)
                    worksheet.write(rows, 13, categ_prod_ending_qty, style=total_value_style)
                    worksheet.write(rows, 14, categ_val_ending_qty, style=header_style)
                    ...
                    prod_qty_pjmin += categ_prod_qty_pjmin
                    prod_qty_pjmout += categ_prod_qty_pjmout
                    prod_qty_rpro += categ_prod_qty_rpro
