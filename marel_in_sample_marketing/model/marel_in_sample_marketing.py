...
class MarelinSampleMarketing(models.Model):
    _name = 'marelin.sample.marketing'
    
    def get_marelin_sample_marketing_no(self):
        nama_baru = self.env['ir.sequence'].next_by_code('marelin.sample.marketing.no')
        return nama_baru
    
    name = fields.Char(string='Id Sample',required=True,copy=False, default=get_marelin_sample_marketing_no,readonly=True )
    ...
    gramasi_option = fields.Selection(string="Gramasi", selection=[
        ('Ya', 'Ya'),
        ('Tidak', 'Tidak'),])
