...
class report_account_aged_partner(models.AbstractModel):
    ...
    def get_lines(self, options, line_id=None):
        ...
        default_branch = [self.env.context.get('branch_id')]
        branch_id = default_branch[0]
        results, total, amls = self.env['report.account.report_agedpartnerbalance'].with_context(include_nullified_amount=True)._get_partner_move_lines(account_types, self._context['date_to'], 'posted', 30, branch_id)
