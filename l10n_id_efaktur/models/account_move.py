...
class AccountMove(models.Model):
    ...
    l10n_id_replace_invoice_id = fields.Many2one('account.invoice', string="Replace Invoice",  domain="['|', '&', '&', ('state', '=', 'posted'), ('partner_id', '=', partner_id), ('reversal_move_id', '!=', False), ('state', '=', 'cancel')]", copy=False)
    ...
    tax_vendor_number = fields.Char(string="Tax Vendor")
    ...
    def _generate_efaktur_invoice(self, delimiter):
        ...
        for invoice in self:
            ...
            for move in invoice.filtered(lambda m: m.state not in ('draft','cancel')):
                ...
                if move.l10n_id_replace_invoice_id:
                    number_ref = str(move.l10n_id_replace_invoice_id.move_name) + " replaced by " + str(move.move_name) + " " + nik
                else:
                    number_ref = str(move.move_name) + " " + nik
                ...
                eTax['MASA_PAJAK'] = date_invoice.month
                eTax['TAHUN_PAJAK'] = date_invoice.year
                eTax['TANGGAL_FAKTUR'] = '{0}/{1}/{2}'.format(date_invoice.day, date_invoice.month, date_invoice.year)
                ...
                lines = move.invoice_line_ids.filtered(lambda x: x.product_id.id == int(dp_product_id) and x.price_unit < 0 and not x.display_type)
                eTax['FG_UANG_MUKA'] = 0
    ...
    def _generate_efaktur(self, delimiter):
        ...
        attachment = self.env['ir.attachment'].create({
            'datas': out,
            'name': 'efaktur_%s.csv' % (fields.Datetime.to_string(datetime.now()).replace(" ", "_")),
            'type': 'binary',
        })
