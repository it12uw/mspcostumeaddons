...
class ProductionBomAksesorisReportReq(models.AbstractModel):
    """
    Class untuk Custom Data Report sebelum di render ke dalam template
    """
    _name = 'report.manufacturing_bom_rekap_req.report_bom_aksesoris_req'
    ...
    def get_report_values(self, docids, data=None):
        ...
        for order in mo:
            for record in order.move_raw_ids:
                if record.product_id.product_tmpl_id.categ_id.id==70 or record.product_id.product_tmpl_id.categ_id.id==106:
