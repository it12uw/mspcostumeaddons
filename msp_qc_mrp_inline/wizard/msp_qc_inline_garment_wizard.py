...
class MspQcinlineGarmentWizard(models.TransientModel):
    _name = "msp.qcinline.garment.wizard"

    def _default_msp_qcinline_garment_wizard(self):
        return self.env['mrp.workorder'].browse(self._context.get('active_ids'))

    name = fields.Many2one('mrp.workorder',string="Name",default=_default_msp_qcinline_garment_wizard, readonly=True, store=True,)
    ...
    @api.multi
    def msp_qc_inline_garment_wizard(self, vals):
        obj_nilai = self.env['msp.qc.inline.garment'].create({
        ...
        return super(MspQcinlineGarmentWizard, self).write(vals)
