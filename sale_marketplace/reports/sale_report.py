...
#     amt_to_invoice = fields.Float('Untaxed Amount To Invoice', readonly=True)
#     amt_invoiced = fields.Float('Untaxed Amount Invoiced', readonly=True)
...
#             sum(l.amt_to_invoice / CASE COALESCE(s.currency_rate, 0) WHEN 0 THEN 1.0 ELSE s.currency_rate END) as amt_to_invoice,
#             sum(l.amt_invoiced / CASE COALESCE(s.currency_rate, 0) WHEN 0 THEN 1.0 ELSE s.currency_rate END) as amt_invoiced,
#             count(*) as nbr,
#             s.name as name,
...
#             s.id as order_id,
#             s.sale_marketplace as market_place_id,
#             l.name as slabel
#         """
...
class SaleReport(models.Model):
    ...
    def _select(self):
        return super(SaleReport, self)._select() + ", s.sale_marketplace as market_place_id ,l.name as slabel, count (distinct s.id) as order_id"
    

    def _group_by(self):
        return super(SaleReport, self)._group_by() + ", s.sale_marketplace, l.name, s.id"