...
class PengecekanSample(models.Model):
    ...
    def _onchange_marel_sample_id(self):
        if self.marel_sample_id and self.marel_sample_id.state == 'done':
            # Menghitung dan mengatur nilai field-field berdasarkan logika yang Anda butuhkan
            ...
            self.hell_relaxed_out_x = self.marel_sample_id.hell_relaxed_out_x
            self.hell_relaxed_out_y = self.marel_sample_id.hell_relaxed_out_y
            ...
            if self.marel_sample_id and self.marel_sample_id.state != 'done':
                warning_msg = "Sample Yang Anda Masukkan Tidak Sesuai, Pilih Sample Dengan Status Done"
