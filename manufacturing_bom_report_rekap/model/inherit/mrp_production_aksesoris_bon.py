...
class ProductionBomAksesorisReportBon(models.AbstractModel):
    """
    Class untuk Custom Data Report sebelum di render ke dalam template
    """
    _name = 'report.manufacturing_bom_report_rekap.report_bon_aksesoris'
    ...
    def get_report_values(self, docids, data=None):
        ...
        for order in mo:
            for record in order.move_raw_ids:
                if record.product_id.product_tmpl_id.categ_id.id==5 or record.product_id.product_tmpl_id.categ_id.id==70 or record.product_id.product_tmpl_id.categ_id.id==105 or record.product_id.product_tmpl_id.categ_id.id==106 or record.product_id.product_tmpl_id.categ_id.id==107 or record.product_id.product_tmpl_id.categ_id.id==109:
        ...
        docargs = {
            ...
            'delivery_date' : ", ".join(order.sale_id2.delivery_date for order in mo),
            'no_so' : ", ".join(order.sale_id2.name for order in mo),
            'nama_customer' : ", ".join(order.sale_id2.partner_id.name for order in mo),
